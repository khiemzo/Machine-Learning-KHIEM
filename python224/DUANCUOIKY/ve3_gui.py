...
def plot_chart():
    ...
    # Kiểm tra đầu vào
    if len(selected_cities) != 1 or len(selected_data) != 1:
        messagebox.showwarning("Lỗi", "Chỉ hỗ trợ vẽ biểu đồ khi chọn một thành phố và một loại dữ liệu!")
        return
    ...
    plt.plot(month_names, values, marker='o', linestyle='-', color='b')
    ...
    # Hiển thị biểu đồ
    messagebox.showinfo("Thông báo", f"Đang hiển thị biểu đồ cho thành phố '{city}' và loại dữ liệu '{dtype}'.")
    plt.show()
