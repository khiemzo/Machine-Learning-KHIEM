...
def plot_climate_data(data, city, dtype):
    """Vẽ biểu đồ dữ liệu khí hậu."""
    month_names = [
        "January", "February", "March", "April", "May", "June",
        "July", "August", "September", "October", "November", "December"
    ]

    # Lấy dữ liệu cho thành phố và loại dữ liệu cụ thể
    values = data.get(city, {}).get(dtype, [])
    if not values:
    ...
    # Hiển thị biểu đồ
    print(f"\nĐang hiển thị biểu đồ cho thành phố '{city}' và loại dữ liệu '{dtype}'.")
...
def main():
    ...
    while True:
        ...
        if len(selected_cities) == 1 and len(selected_data) == 1:
            ...
            plot_climate_data(data, city, dtype)
        else:
            print("\n[!] Cảnh báo: Chỉ hỗ trợ vẽ biểu đồ khi chọn một thành phố và một loại dữ liệu.")
