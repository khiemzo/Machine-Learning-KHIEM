...
def plot_multiple_cities(data, cities, dtype, months):
    ...
    messagebox.showinfo("Thông báo", f"Đang hiển thị biểu đồ so sánh {dtype.upper()} giữa các thành phố.")
    plt.show()
...
def plot_multiple_data_types(data, city, data_types, months):
    ...
    for dtype in data_types:
        values = data.get(city, {}).get(dtype, [])
        if not values:
            messagebox.showwarning("Lỗi", f"Không có dữ liệu cho thành phố '{city}' và loại dữ liệu '{dtype}'.")
            continue
        ...
        messagebox.showinfo("Thông báo", f"Đang hiển thị biểu đồ {dtype.upper()} cho thành phố '{city}'.")
        plt.show()
...
def plot_chart():
    ...
    if len(selected_cities) > 1 and len(selected_data) == 1:
        ...
        messagebox.showwarning("Lỗi", "Chỉ hỗ trợ vẽ biểu đồ khi:")
        messagebox.showwarning("Lỗi", "- Nhiều thành phố và một loại dữ liệu, hoặc")
        messagebox.showwarning("Lỗi", "- Một thành phố và nhiều loại dữ liệu.")
...
# Nút hiển thị dữ liệu và vẽ biểu đồ
...
plot_button = tk.Button(button_frame, text="Vẽ Biểu Đồ", command=plot_chart)
